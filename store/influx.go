...
func Ingest(data []map[string]interface{}) {

	writeAPI := client.WriteAPIBlocking(organizationName, bucketName)

	err := insertData(writeAPI, data)
	if err != nil {
		log.Fatal(err)
	}
...
func insertData(writeAPI api.WriteAPIBlocking, data []map[string]interface{}) error {

	for _, stockData := range data {
		// Create a new point
		point := influxdb2.NewPointWithMeasurement("prices").
			AddTag("SecurityID", strconv.FormatInt(int64(stockData["security_id"].(float64)), 10)).
			AddField("Price", stockData["last_price"].(float64)).
			AddField("CZG", stockData["change_percent"].(float64)).
			SetTime(time.Now())
